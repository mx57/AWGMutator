...
- **Авто-переключение зеркал Cloudflare:** Интеллектуальный роутинг запросов регистрации через пул из 12+ рабочих зеркал API и IP-шлюзов с автоматическим фолбэком при блокировке основного домена.
- **Встроенная криптография:** Генерация пар ключей Curve25519 и регистрация профиля WARP/WARP+.
- **Инъекция обфускации:** Мгновенное наложение случайных Anti-DPI заголовков на WARP-профиль для обхода блокировок WireGuard.

### 3. 🌐 Каталог из 18 антицензурных DNS-серверов и Генетический отбор DNS
- **18 проверенных DNS провайдеров:** Cloudflare, Google DNS, Quad9 (Uncensored/Clean), AdGuard DNS (Default & Family), Control D (Unfiltered & Malware-block), Mullvad DNS, NextDNS, CleanBrowsing, OpenDNS, Comodo Secure, Level3, DNS.WATCH, CIRA Canadian Shield.
- **Эволюция DNS:** Генетический алгоритм мутирует и подбирает наиболее быстрый и неблокируемый DNS-резолвер для каждого поколения.

### 4. 🚀 Реальный User-Space TUN Packet Router & Проксирование
- **Настоящая маршрутизация пакетов:** Встроенный `TunPacketRouter` выполняет чтение и сборку сырых IPv4/UDP пакетов на Android `VpnService` TUN-интерфейсе.
- **Защита сокетов (Socket Protection):** Автоматический вызов `vpnService.protect(socket)` для предотвращения рекурсивных петель маршрутизации.
- **Реальный DNS прокси и TCP/UDP Handshake Ping:** Замер честной задержки (RTT) до эндпоинтов и DNS-серверов через реальные сокетные соединения.

### 5. 🔬 Лаборатория Anti-DPI (Deep Packet Modulation)
